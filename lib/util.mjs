...
export function urlJoin(base, route) {
  ...
  // Normalize double-slash: protocol-relative routes (//foo) or i18n expansion artifacts
  // produce invalid URLs like https://host//foo. Collapse any run of 2+ slashes after host.
  return `${b}${r}`.replace(/([^:])\/\/+/g, '$1/');
}
...
export function expandLocaleRoutes(routes, i18n) {
  ...
  // BUG-R7-07: warn on unknown placeholders — they stay verbatim and produce wrong test routes.
  const unknownPlaceholders = (pattern.match(/\{[^}]+\}/g) || []).filter((p) => p !== '{locale}' && p !== '{route}');
  if (unknownPlaceholders.length > 0) {
    process.stderr.write(`[rm] WARN: i18n.url_pattern contains unknown placeholders ${unknownPlaceholders.join(', ')} — they will appear verbatim in test routes\n`);
  }
  const out = [];
