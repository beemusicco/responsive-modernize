...
export function adjustForContrast(fg, bg, target = 4.5) {
  ...
  const towardWhite = bgLum < 0.5; // light bg → darken fg; dark bg → lighten fg (logic inverted)
  // Actually: dark bg (lum < 0.5) → fg should go LIGHTER (toward white) for contrast
...
export function colorToString(c) {
  const hex = '#' + [c.r, c.g, c.b].map((v) => Math.max(0, Math.min(255, v)).toString(16).padStart(2, '0')).join('');
  return c.a < 1 ? `rgba(${c.r}, ${c.g}, ${c.b}, ${c.a.toFixed(2)})` : hex;
}
