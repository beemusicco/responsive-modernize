...
export async function runVerify({brief, briefDir, outDir, viewports, engines, dryRun}) {
  ...
  if (aiCfg.enabled && regressions.length > 0) {
    ...
    // Reclassify: regressions where AI says NOT a regression get filtered out of fail count
    const realRegressions = regressions.filter((r) => !r.aiJudge || r.aiJudge.isRegression !== false);
    log(`  ai-diff: ${realRegressions.length}/${regressions.length} confirmed real regressions (rest are intended improvements)`);
  }
