...
export async function aiJudgeDiff({baselinePath, verifyPath, diffPath, pctDiff, threshold, route, viewport, briefDir, timeoutSec = 60}) {
  ...
  const briefBaseline = relative(briefDir, baselinePath);
  const briefVerify = relative(briefDir, verifyPath);
  const briefDiff = diffPath && existsSync(diffPath) ? relative(briefDir, diffPath) : null;
