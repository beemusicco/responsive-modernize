...
const OBJECT_STYLE_CALLEES = ['stylex.create', 'stylex.props', 'css', 'styleVariants', 'cva', 'styled'];
const UNITLESS_NUMERIC_CSS_PROPS = new Set(['line-height', 'opacity', 'z-index', 'font-weight', 'flex', 'flex-grow', 'flex-shrink', 'order']);
...
async function scanCSSInJS(filePath, projectRoot) {
  ...
  if (/from\s+['"][^'"]*styled-system\/css/.test(text)) runtimes.push('panda-css');
    ...
    const objectStyleCalleePattern = OBJECT_STYLE_CALLEES.map(escapeRegExp).join('|');
    const objectStyleCallRe = new RegExp(`(?:${objectStyleCalleePattern})\\s*\\(`);
    // Cheap precheck — any of the supported runtime markers
    if (
      runtimes.length === 0 &&
      !/(styled[.\(]|\bcss`|\btw`|useStyles\$\s*\()/.test(text) &&
      !objectStyleCallRe.test(text)
  ...
  // wrapper that postcss CAN parse.
    const OBJECT_STYLE_RE = new RegExp(`(?:${objectStyleCalleePattern})\\s*\\([^)]*?\\{([\\s\\S]*?)\\}\\s*\\)`, 'g');
  ...
  while ((om = OBJECT_STYLE_RE.exec(text))) {
    ...
    while ((km = KV_RE.exec(objBody))) {
        ...
        if (/^\d+(\.\d+)?$/.test(val)) {
          if (UNITLESS_NUMERIC_CSS_PROPS.has(cssProp) || Number(val) < 2) continue;
          val = `${val}px`;
        }
...
async function scanHTML(filePath, projectRoot) {
  ...
  // <table> without responsive class — typical legacy WP/Shopify problem on mobile
  const tableCount = $('table').length;
  if (tableCount > 0) {
    let tablesNeedingResponsive = 0;
    $('table').each((_, el) => {
      const $el = $(el);
      const cls = $el.attr('class') || '';
      if (!/rm-card-on-mobile|responsive|mobile/.test(cls)) tablesNeedingResponsive++;
    });
    if (tablesNeedingResponsive > 0) {
      pushIssue(issues, {
        kind: 'table-not-responsive',
        file: rel,
        line: 0,
        severity: 'warn',
        msg: `${tablesNeedingResponsive} <table> without responsive class — wide tables horizontally scroll on mobile.`,
        data: {count: tablesNeedingResponsive},
        autoFixable: true,
        fix: 'table-to-cards',
      });
    }
