...
const audit = () => {
  ...
  const getEffectiveBg = (el) => {
    ...
    while (cur && cur !== document.body) {
      ...
      if (bg && cs.backgroundColor !== 'rgba(0, 0, 0, 0)' && bg.join() !== '0,0,0') return bg;
      cur = cur.parentElement;
    }
    return [255, 255, 255];
  };
  ...
  for (const el of document.querySelectorAll(textSel)) {
    ...
    if (ratio < 4.5 && contrastViolations.length < 6) {
      ...
      contrastViolations.push({selector: sel, tag: el.tagName.toLowerCase(), ratio: ratio.toFixed(2), text: el.textContent.trim().slice(0, 50)});
    }
  }
