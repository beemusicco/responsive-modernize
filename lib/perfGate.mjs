  ...
  for (const route of routes) {
    ...
    try {
      ...
      const cwv = await page.evaluate(() => new Promise((resolve) => {
        ...
        try {
          // INP proxy: worst interaction latency observed. Event-timing only fires for
          // trusted interactions, so on a no-interaction load this stays null (correctly
          // un-failable) — but the threshold below is now wired, not dead.
          new PerformanceObserver((list) => {
            for (const e of list.getEntries()) {
              if (out.INP == null || e.duration > out.INP) out.INP = e.duration;
            }
      ...
      if (cwv.INP != null && cwv.INP > INP_MAX) fail.push(`INP ${Math.round(cwv.INP)}ms > ${INP_MAX}`);
      results.push({route, cwv, fail});
      if (fail.length) failures.push({route, fail, cwv});
    } catch (e) {
      // A route that never loaded (goto timeout / nav error) is a gate FAILURE, not a
      // silent pass — otherwise the slowest pages (which never settle) report "0 fail".
      const fail = [`load-error: ${e.message}`];
      results.push({route, error: e.message, fail});
      failures.push({route, fail, error: e.message});
    }
    await ctx.close();
  }
  await browser.close();
