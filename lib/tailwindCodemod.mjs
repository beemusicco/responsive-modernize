...
export async function tailwindLayoutStackCodemod({briefDir}) {
  ...
  for (const rel of files) {
    ...
    const result = walkJSX(src, (ctx) => {
      // Guard: skip if ANY ancestor is a semantic horizontal parent.
      if (ctx.ancestorTags.some((t) => SEMANTIC_HORIZONTAL_PARENTS.has(t))) return null;
      // Also skip if the element itself is one of those tags.
      if (SEMANTIC_HORIZONTAL_PARENTS.has(ctx.tagName)) return null;
