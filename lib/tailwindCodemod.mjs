...
// Split Tailwind class strings on real separators only. Arbitrary values can
// contain whitespace inside brackets, e.g. before:content-["x y"].
function splitClassTokens(classes) {
  const tokens = [];
  let token = '';
  let bracketDepth = 0;
  let quote = null;
  for (let i = 0; i < classes.length; i++) {
    ...
    if (ch === '\\') {
      token += ch;
      if (i + 1 < classes.length) token += classes[++i];
      continue;
    }
    if (quote) {
      token += ch;
...
function sanitizeTouchClasses(classes, opts = {}) {
  ...
  const tokens = splitClassTokens(classes);
  const out = [];
  let touched = false;
...
import {walkJSX} from './jsxWalker.mjs';
...
/**
 * v1.14.0: AST-tree walker replaces regex+token guard.
 * Skips elements where ancestor chain contains nav/menu/header/footer/aside-as-nav —
 * those keep semantic info in PARENT tag, not className tokens (utility-Tailwind).
 * Verified against viagoshop-v2 mobile-bottom-nav regression.
 */
const SEMANTIC_HORIZONTAL_PARENTS = new Set(['nav', 'menu', 'header', 'footer']);

/**
 * v1.14.1 FIX (C3): React apps use PascalCase components for navigation:
 * <Navbar>, <NavMenu>, <Header>, <TopBar>, <SiteHeader>. Lowercase-only
 * tag check missed these → child div with grid-cols-N got stacked, breaking
 * mobile navs in real React codebases. Heuristic: any tag whose name contains
 * Nav, Menu, Header, Footer, Topbar substring (case-insensitive) is treated
 * as a semantic horizontal parent. Trade-off: false-positive on something
 * like <NavigationReport> (rare). Configurable via brief.semanticHorizontalRe
 * regex override.
 */
const PASCAL_NAV_RE = /(Nav|Menu|Header|Footer|Topbar|Sidebar|Toolbar|Breadcrumb)/i;
function isSemanticHorizontalAncestor(tagName, briefRe) {
  if (!tagName) return false;
  if (SEMANTIC_HORIZONTAL_PARENTS.has(tagName)) return true;
  // PascalCase / component-style: first char uppercase
  if (tagName[0] >= 'A' && tagName[0] <= 'Z') {
    if (briefRe instanceof RegExp) return briefRe.test(tagName);
    return PASCAL_NAV_RE.test(tagName);
  }
  return false;
}
...
export async function tailwindLayoutStackCodemod({briefDir}) {
  ...
  for (const rel of files) {
    ...
    const src = await readFile(full, 'utf8');
    let fileEdits = 0;
    const result = walkJSX(src, (ctx) => {
      // v1.14.1 FIX (C3): include PascalCase Nav/Header/Footer components in guard.
      if (ctx.ancestorTags.some((t) => isSemanticHorizontalAncestor(t))) return null;
      if (isSemanticHorizontalAncestor(ctx.tagName)) return null;
      // Find className attr; transform only string literal value (skip {…} expressions for safety).
      const classNameAttr = ctx.attrs.find((a) => a.name === 'className');
      if (!classNameAttr || classNameAttr.kind !== 'string') return null;
      const rawValue = classNameAttr.valueRaw;
      const quote = rawValue[0];
      const inner = rawValue.slice(1, -1);
      const {classes: newClasses, touched} = stackLayoutClasses(inner);
      if (!touched) return null;
      // Build replacement attrs: must regenerate ALL attrs since AST walker replaces full attr span
      const newAttrs = ctx.attrs.map((a) => {
        if (a.name === '__spread__') return a.valueRaw;
        if (a.kind === 'boolean') return a.name;
        if (a.name === 'className') return `${a.name}=${quote}${newClasses}${quote}`;
        return `${a.name}=${a.valueRaw}`;
      }).join(' ');
      fileEdits++;
      return {newAttrs: ' ' + newAttrs};
    });
    if (result.parseError) {
      // v1.14.1 FIX: warn loudly so silent-no-op doesn't look like success.
      // .tsx with TS-only syntax (decorators, satisfies, generics) fails acorn-jsx parse.
      // Operator needs to see this so they can fall back to per-file regex pass or skip TS-only files.
      console.warn(`[rm:jsx-parse] skipped ${rel}: ${result.parseError.slice(0, 100)}`);
      parseErrors.push({file: rel, error: result.parseError.slice(0, 80)});
      continue;
    }
    if (result.edits > 0) {
      await safeWrite(full, result.out);
      touchedFiles++;
      totalEdits += result.edits;
      edits.push({file: rel, edits: result.edits});
    }
  }
  return {touchedFiles, totalEdits, edits, parseErrors};
}
