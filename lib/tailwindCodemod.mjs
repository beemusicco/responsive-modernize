...
export async function tailwindNavHamburgerCodemod({briefDir}) {
  ...
  let touchedFiles = 0, totalEdits = 0;
  const edits = [];
  // Match opening <nav ...> and closing </nav>, capture inner content
  const NAV_RE = /<nav\b([^>]*?)>([\s\S]*?)<\/nav>/g;
  for (const rel of files) {
    const full = join(briefDir, rel);
    const src = await readFile(full, 'utf8');
    let fileEdits = 0;
    let toggleId = 0;
    const out = src.replace(NAV_RE, (match, attrs, children) => {
      // Idempotency: skip if already transformed
      if (/data-rm-hamburger|rm-nav-toggle/.test(attrs) || /rm-nav-toggle/.test(children)) return match;
      // Count item-like children
      const itemCount = (children.match(/<\s*(a|Link|button)\b/gi) || []).length;
      if (itemCount < 5) return match;
      // Skip if children include complex JSX expressions ({...}) at top level — unsafe
      // (heuristic: spread props in children that aren't inside a tag)
      const id = `rm-nav-toggle-${++toggleId}`;
      // Add data-rm-hamburger to attrs
      ...
      fileEdits++;
      return `<nav${newAttrs}>${head}${wrapped}</nav>`;
    });
    if (fileEdits > 0) {
      await safeWrite(full, out);
      touchedFiles++;
