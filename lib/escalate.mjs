...
const KIND_PLAYBOOK = {
  'nav-needs-hamburger': {
    title: 'Top-level <nav> has too many items for mobile',
    why: 'Mobile screens (≤430px) can only fit 2-3 nav items before they wrap or overflow. >4 items = use hamburger drawer pattern.',
    fix_strategy: 'For each sample selector: find the JSX <nav>. Wrap visible items in `<div className="hidden md:flex gap-N">…</div>`. Add a mobile-only hamburger button `<button className="md:hidden" aria-label="Menu">☰</button>` with state to toggle a `<div className="md:hidden ...">` drawer. Preserve all current items in the drawer. Use existing patterns in the codebase if available (search for `useState.*open`, `Drawer`, `MobileMenu`).',
  },
  'hero-needs-rebalance': {
    title: 'Hero section is desktop-stacked (image+text side by side)',
    why: 'Hero 2-col grid on desktop becomes cramped on mobile. Stack visually (text above image on mobile, or based on importance) and reduce text+heading size proportionally.',
    fix_strategy: 'Find the hero element (grep for `hero`, `Hero`, `landing`). Add Tailwind `flex-col md:flex-row` or `grid-cols-1 md:grid-cols-2`. For headings, ensure they use the Utopia `--step-*` scale (already injected globally). If image goes second on mobile, add `order-last md:order-none` or similar.',
  },
  'content-hierarchy-mobile': {
    title: 'Content hierarchy not adjusted for mobile',
    why: 'Information-dense sections (long paragraphs, tertiary metadata, secondary nav) overwhelm mobile users.',
    fix_strategy: 'For each section: identify tertiary content (timestamps, byline metadata, supporting paragraphs) and add `hidden md:block` so it disappears below 768px. For long paragraphs, consider wrapping in `<details>` for collapsible content on mobile only. Use brand voice from .claude-stack.json to decide what to keep prominent.',
  },
  'layout-not-responsive': {
    title: 'Layout renders desktop-width on mobile viewport',
    why: 'A `grid grid-cols-3` without a `md:` prefix forces 3 columns on a 360px phone — each cell becomes ~110px wide and content is unreadable. Same for `flex-row` with 3+ children.',
    fix_strategy: 'For EACH sample.selector: grep src/ for the matching className, then REWRITE: `grid-cols-N` (N≥2) → `grid-cols-1 md:grid-cols-N`. `flex-row` → `flex-col md:flex-row`. If you find responsive variants ALREADY in the file (e.g. `lg:grid-cols-4`), keep them. Do NOT modify navigation bars or carousel tracks (those are intentionally horizontal even on mobile — check parent for `nav`, `menu`, `carousel`, `swiper`, `marquee` classes before editing).',
  },
 ...
 */
export async function runAutoImpeccable({briefPath, briefDir, timeoutSec = 600}) {
  ...
  const t0 = Date.now();
    ...
    return new Promise((resolve) => {
      // NOTE: Do NOT pass --bare — it skips keychain reads, breaking OAuth lookup.
      // Brief content goes via stdin (--print expects prompt on stdin or as -p arg).
      const args = [
        '--print',
      '--add-dir', briefDir,
      ...
      log(`  spawning: claude ${args.join(' ')} <BRIEF on stdin>`);
      let stdout = '', stderr = '';
      let resolved = false;
      let timer;
      const finish = (val) => {
      ...
      child.stdin.write(briefContent);
      child.stdin.end();
      timer = setTimeout(() => {
      ...
      child.on('error', (e) => {
        ...
        finish({skipped: true, reason: `spawn error: ${e.message}`});
      });
      child.on('exit', (code) => {
