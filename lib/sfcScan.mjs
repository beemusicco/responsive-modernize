...
// Blank out <script>…</script> bodies and HTML comments before matching
// <style> blocks, so a `<style>` literal inside a script string (Svelte/Vue)
// or a commented-out block is not mis-detected as a real style block.
// Replaces every non-newline char with a space → preserves byte offsets and
// line numbers so issue locality stays accurate.
function maskNonStyleRegions(text) {
...
export async function scanSFC(filePath, projectRoot) {
  ...
  const masked = maskNonStyleRegions(text);
  ...
  while ((m = STYLE_BLOCK_RE.exec(masked))) {
