...
export function walkJSX(src, visitor) {
    ...
    if (node.type === 'JSXElement' || node.type === 'JSXFragment') {
      ...
      if (!isFragment) {
        ...
        attrs = node.openingElement.attributes.map((attr) => {
          ...
          // v1.14.1 FIX (C1): JSXNamespacedName has .name as JSXIdentifier OBJECT, not string.
          // Shortcut `attr.name.name` returned that object → string-concat'd as "[object Object]"
          // in attr reconstruction → corrupt SVG xlink:href etc. Always go through nameOf().
  ...
  // v1.14.1 FIX: overlap-skip guard. If two edits overlap (outer fullReplacement
  // covers inner edit), inner.start < cursor → src.slice(cursor, inner.start)
  // emits empty + inner.replacement appended after outer = JSX garbage.
  // Drop the inner edit silently. Visitors using fullReplacement must accept
  // that nested edits won't apply within that span.
  let out = '';
  let cursor = 0;
  let droppedOverlaps = 0;
  for (const e of edits) {
    if (e.start < cursor) { droppedOverlaps++; continue; }
    out += src.slice(cursor, e.start) + e.replacement;
    cursor = e.end;
  }
  out += src.slice(cursor);
  return {out, edits: edits.length - droppedOverlaps, droppedOverlaps};
}
