            ...
            if (brief.dismissCookieBanner !== false) {
              await page.evaluate(() => {
                const selectors = [
                  ...
                  'button[aria-label*="accept" i]', 'button:has-text("Accept")',
                  '#cookie-accept', '.cookie-accept', '[data-cy*="cookie-accept"]',
                ];
                for (const s of selectors) {
                  try { const b = document.querySelector(s); if (b) { b.click(); return; } } catch {}
                }
                // Slovenian + EU common patterns
                const buttons = document.querySelectorAll('button, a[role="button"]');
                for (const b of buttons) {
                  const t = (b.textContent || '').trim().toLowerCase();
                  if (/sprejmi|accept|allow|ok|got it|razumem/.test(t)) { try { b.click(); return; } catch {} }
                }
              });
