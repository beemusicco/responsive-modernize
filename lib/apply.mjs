...
const HANDLERS = {
  ...
  'table-to-cards': async ({briefDir}) => {
    // Inject responsive CSS into largest .css file that converts <table> to card view
    // on mobile via display:block + ::before content from data-label attribute.
    // Caller is responsible for adding data-label to <td> in JSX/HTML (semantic).
    const {globby} = await import('globby');
    const cssCandidates = await globby(['**/*.css', '!node_modules/**', '!.responsive-modernize/**', '!.next/**'], {cwd: briefDir, gitignore: true});
    if (cssCandidates.length === 0) return {applied: false, reason: 'no .css file to append rule to'};
    const {stat: fsStat} = await import('fs/promises');
    ...
    const css = await readFile(target, 'utf8');
    if (css.includes('table-to-cards responsive')) return {applied: false, reason: 'rule already injected'};
    const block = `
...
`;
    ...
    return {applied: true, before: css, after: css + block, changed: 1, target};
  },
