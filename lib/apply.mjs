...
import {tailwindTouchTargetCodemod, tailwindSafeAreaCodemod, tailwindLayoutStackCodemod, tailwindFormStackCodemod, tailwindSidebarDrawerCodemod, tailwindNavHamburgerCodemod} from './tailwindCodemod.mjs';
...
const HANDLERS = {
  ...
  'truncate-text-overflow': async ({briefDir, issue}) => {
    ...
    const cssCandidates = await globby(['**/*.css', '!node_modules/**', '!.responsive-modernize/**', '!.next/**', '!dist/**', '!build/**', '!out/**', '!.cache/**', '!.turbo/**'], {cwd: briefDir, gitignore: true});
  ...
  'tailwind-touch-target': async ({briefDir, opts}) => {
    // v1.14.6 FIX: real-world test on libro caught this codemod destroying custom-sized
    // toggles (h-5 w-9 → min-h-11 min-w-11 grew 20×36 button to 44×44). Same class as
    // octanorm .group bug — class-rewrite assumes semantic intent that doesn't hold for
    // custom UI primitives. Now gated --aggressive opt-in (consistent with layout codemods).
    if (!opts?.aggressive) return {applied: false, reason: '--aggressive required (rewrites h-N w-N classes — may grow custom-sized buttons)'};
  ...
  'tailwind-nav-hamburger': async ({briefDir, opts}) => {
    // Layout-impacting — opt-in via --aggressive
    if (!opts?.aggressive) return {applied: false, reason: '--aggressive required (layout-impacting nav wrapper)'};
    const r = await tailwindNavHamburgerCodemod({briefDir});
    if (r.totalEdits === 0) return {applied: false, reason: 'no <nav> with ≥5 link items found'};
    return {applied: true, before: '', after: '', changed: r.totalEdits, target: `${r.touchedFiles} .tsx/.jsx files`};
  },
  ...
  'fix-low-color-contrast': async ({briefDir, issue}) => {
    ...
    let css = await readFile(target, 'utf8');
    let root;
    // v1.13.3: log parse target so operator can identify the malformed CSS file.
    try { root = postcss.parse(css, {from: target}); } catch (e) { log(`  [rm:contrast-parse] ${target}: ${e.message.split('\n')[0]}`); return {applied: false, reason: `css parse error in ${target}`}; }
    let changed = 0;
    // v1.13.1 FIX: build per-sample bg map (default white) — diagnose now passes bg in sample.bg.
    // v1.13.1 FIX: exact selector match — substring match was polluting unrelated CSS rules.
    const sampleByExactSel = new Map();
    for (const s of samples) {
      sampleByExactSel.set(s.selector, s);
    }
    root.walkRules((rule) => {
      // postcss `rule.selectors` splits comma-separated selectors. Match ANY exact selector.
      const sels = rule.selectors || [rule.selector];
      let matched = null;
      for (const sel of sels) {
        if (sampleByExactSel.has(sel.trim())) { matched = sampleByExactSel.get(sel.trim()); break; }
      }
      if (!matched) return;
      rule.walkDecls('color', (decl) => {
        // v1.13.1 FIX: idempotency check matches the ACTUAL marker we write (was checking wrong string).
        if (decl.value.includes('--rm-contrast-fixed')) return;
        const fg = parseColor(decl.value.replace(/\s*\/\*.*?\*\//g, '').trim());
        if (!fg) return;
        // v1.13.1 FIX: respect detected bg from diagnose (was hardcoded white — broke dark themes).
        let bg = {r: 255, g: 255, b: 255, a: 1};
        if (matched.bg && Array.isArray(matched.bg) && matched.bg.length >= 3) {
          bg = {r: matched.bg[0], g: matched.bg[1], b: matched.bg[2], a: 1};
        }
        if (contrastRatio(fg, bg) >= 4.5) return;
        const adj = adjustForContrast(fg, bg, 4.5);
        if (!adj) return;
        decl.value = colorToString(adj) + ' /* --rm-contrast-fixed */';
        decl.raws.before = decl.raws.before || ' ';
        changed++;
      });
    });
  ...
  'add-focus-visible-rules': async ({briefDir}) => {
    ...
    for (const rel of cssCandidates) {
      ...
      root.walkRules((rule) => {
        ...
        // v1.13.1 FIX: skip if selector ALREADY has :focus-visible (would produce :focus-visible:focus-visible).
        if (/:focus-visible\b/.test(rule.selector)) return;
  ...
  'add-pwa-manifest': async ({briefDir}) => {
    ...
    const {globby} = await import('globby');
    const htmlFiles = await globby(['**/*.html', '!node_modules/**', '!.responsive-modernize/**', '!.next/**', '!dist/**', '!build/**', '!out/**', '!.cache/**', '!.turbo/**'], {cwd: briefDir, gitignore: true});
    ...
    for (const rel of htmlFiles) {
      ...
      for (const el of imgs) {
        ...
        try {
          ...
          fileTouched = true;
        } catch (e) { log(`  [rm:apply-srcset] sharp failed on ${src}: ${e.message}`); }
      }
      if (fileTouched) {
        await safeWrite(full, $.html());
        touched++;
